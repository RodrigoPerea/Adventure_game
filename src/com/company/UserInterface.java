    ...
    do {
      ...
      if (input.equals("look")) {
        ...
        if (currentRoom instanceof Room) {
          ((Room) currentRoom).viewRoomContent();
          System.out.println("Take the thing 0/1/2/3 .. from the room : ");
          takeItem = Integer.parseInt(String.valueOf(input.lines()));
          Item t = ((Room) currentRoom).getRoomItem(takeItem);
          player.picUpItem(t);
          player.viewinventoryList();
        }
        ...
        takeItem = Integer.parseInt(String.valueOf(input.lines()));
        if (currentRoom instanceof Room) {
          Item t = player.getInventoryList().get(takeItem);
          ((Room) currentRoom).addItem(t);
          ((Room) currentRoom).viewRoomContent();
        }
